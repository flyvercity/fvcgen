...
class FVCGenerator:
    ...
    def _write_fvc_content(self, output: TextIO) -> None:
        ...
        # Write FVC footer
        self._write_footer(output, object_count)

    def _write_header(self, output: TextIO) -> None:
        """Write FVC file header."""
        timestamp = datetime.now().isoformat()
        base_point = self.config.general.base_point

        output.write('# FVC Scenario File\n')
        output.write('# Generated by FVC Generator v0.1.0\n')
        output.write(f'# Generated at: {timestamp}\n')
        output.write(f'# Base Point: {base_point.lat:.6f}, {base_point.lon:.6f}, {base_point.alt:.1f}m\n')
        output.write(f'# Time Step: {self.config.general.defaults.time_step}s\n')
        output.write('#\n')
        output.write('# Format: time,object_id,lat,lon,alt,speed,heading\n')
        output.write('#\n')

    def _write_footer(self, output: TextIO, object_count: int) -> None:
        """Write FVC file footer."""
        output.write(f'# End of scenario - {object_count} objects\n')
    ...
    def _write_object_trajectory(self, output: TextIO, origin_idx: int, obj_idx: int, obj_config: ObjectConfig) -> None:
        ...
        for position in trajectory:
            ...
            for obj_id in obj_config.id:
                output.write(f'{position.time:.3f},{obj_id},{lat:.6f},{lon:.6f},{alt:.1f},{position.speed:.1f},{position.heading:.1f}\n')
